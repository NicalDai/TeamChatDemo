...
public class TeamAVChatVoiceMuteDialog extends CustomAlertDialog {
    ...
    public TeamAVChatVoiceMuteDialog(Context context, String teamId, List<Pair<String, Boolean>> voiceMutes) {
        ...
        for (Pair<String, Boolean> voiceMute : voiceMutes) {
            final TeamAVChatVoiceMuteItem item = new TeamAVChatVoiceMuteItem();
            item.setAccount(voiceMute.first);
            item.setMute(voiceMute.second);


            String nick = AVChatKit.getTeamDataProvider().getTeamMemberDisplayName(teamId, item.getAccount());
            ...
            if(nick == null){
                ...
                NIMClient.getService(UserService.class).fetchUserInfo(accounts).setCallback(new RequestCallback<List<NimUserInfo>>() {
                    ...
                    public void onSuccess(List<NimUserInfo> nimUserInfos) {
                        Log.d("收到呼叫请求，刷新群成员资料", "onSuccess: ");
                        item.setDisplayName(nimUserInfos.get(0).getName());
                    }
